...
async function pdfToImage(pdfData, canvas) {
  ...
  return (await printPDF(pdfData)).map(async (c) => {
    canvas.add(
      new fabric.FabricImage(await c, {
        scaleX: scale,
        scaleY: scale,
      })
...
export default function DocumentBuilder({ onBack, data }) {
  useEffect(() => {
    ...
    //   await Promise.all(pdfToImage(e.target.files[0], canvas));
      await Promise.all(await pdfToImage(e.target.files[0], canvas));
      canvas.remove(text);
    };
    ...
    return () => {
      document
        ...
        .removeEventListener("change", evHandler);
    };
  }, []);
