...
const NewDocContainer = ({ types, tags, token }) => {
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">Create A Document</h1>
                ...
                <div className="bg-white rounded-lg w-full mx-auto p-8">
                    {modalType === 'upload' ? (
                        ...
                        <>
                            ...
                            <p className="text-gray-600 mb-6">Please select a file to upload.</p>
                            ...
                            <form className="space-y-6" action={formAction}>
                                ...
                                <div className="border-2 border-dashed border-gray-300 p-6 rounded-lg bg-gray-50 text-center cursor-pointer hover:border-blue-500 transition-colors duration-300">
                                    ...
                                    {selectedFileName && (
                                        <p className="mt-2 text-sm text-gray-700">Selected file: <span className="font-semibold">{selectedFileName}</span></p>
                                    )}
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <fieldset className="border p-4 rounded-lg bg-gray-50">
                                        <legend className="text-gray-700 font-medium">Document Type</legend>
                                        <select name="type" id="type" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                            {types.map((type, indx) => (
                                                <option key={indx} value={type.id} className="text-gray-700">
                                                    {type.name}
                                                </option>
                                            ))}
                                        </select>
                                    ...
                                    <fieldset className="border p-4 rounded-lg bg-gray-50">
                                        ...
                                        {tags.map((tag, indx) => (
                                            <div key={tag.id} className="flex items-center space-x-2 mt-2">
                                                <input name='tag' value={tag.id} type="checkbox" id={indx} className="h-5 w-5 text-blue-600 focus:ring-blue-500" />
                                                <label htmlFor={indx} className="text-gray-700">{tag.name}</label>
                                            </div>
                                        ))}
                                    </fieldset>
                                </div>
                                    ...
                                    >
                                        {isPending ? (
                                            <span className="flex items-center justify-center">
                                                <svg className="animate-spin h-5 w-5 mr-3 border-t-2 border-b-2 border-blue-600 rounded-full" viewBox="0 0 24 24" style={{ borderTopColor: 'transparent' }}>
                                                    <circle cx="12" cy="12" r="10" fill="none" strokeWidth="4"></circle>
                                                    <circle cx="12" cy="12" r="10" fill="none" strokeWidth="4" strokeDasharray="31.4" strokeDashoffset="10" stroke="blue"></circle>
                                                </svg>
                                                Submitting...
                                            </span>
                                        ) : (
                        ...
                        <>
                            <h2 className="text-2xl font-bold mb-6 text-gray-800">Create Blank Document</h2>
                            <p className="text-gray-600 mb-6">Starting a new blank document.</p>
